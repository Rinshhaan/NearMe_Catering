            ...
            if (data.country_code !== 'IN' || (data.region !== 'Kerala' && data.region !== 'KL')) {
                document.body.innerHTML = `
                    <div style="display:flex; height:100vh; width:100vw; justify-content:center; align-items:center; background:#1a1b1e; color:white; font-family:'Plus Jakarta Sans', sans-serif; text-align:center; padding:20px;">
                        <div>
                            <h1 style="color:#e74c3c;">Access Restricted ðŸ“</h1>
        ...
        confirmBtn.onclick = async () => {
            ...
            if (site && site.aTimerEnd) {
                ...
                if (Date.now() >= deadline) {
                    alert("â° Sorry, booking time for this site has expired!");
            ...
            const waMsg = `âœ… *Booking Confirmation â€” Nearby Caters*\n\n` +
                `ðŸ“ *Site:* ${siteName}\n` +
                `ðŸ“… *Date:* ${siteDate}\n` +
                `â° *Time:* ${siteTime}\n` +
                `ðŸ‘¤ *Name:* ${name}\n` +
                `ðŸ“ž *Phone:* ${phone}\n` +
                `ðŸ• *Booked at:* ${bookedAt}`;

            const waUrl = `https://api.whatsapp.com/send?phone=${ADMIN_WHATSAPP}&text=${encodeURIComponent(waMsg)}`;

            // Show alert then redirect
            alert("âœ… Slot booked! Redirecting to WhatsApp to confirm...");
...
window.openDrawer = (id) => {
    ...
    if (drawerBody) {
        ...
        drawerBody.innerHTML = `
            ...
            <p style="color:var(--primary); font-weight:700; margin:8px 0;">ðŸ“ ${s.aPlaceName}</p>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin:15px 0;">
                ${s.aWage ? `<div style="background:#f8f9fa; padding:10px; border-radius:10px;"><small style="color:#666">Wage</small><div style="font-weight:700">â‚¹${s.aWage}</div></div>` : ''}
                ...
                ${s.aGuests ? `<div style="background:#f8f9fa; padding:10px; border-radius:10px;"><small style="color:#666">Guest Count</small><div style="font-weight:700">ðŸ‘¥ ${s.aGuests}</div></div>` : ''}
            ...
            ${s.aNotes ? `<div style="background:#f8f9fa; padding:15px; border-radius:10px; margin-bottom:15px;">
                <h4 style="margin:0 0 10px 0;">ðŸ“Œ Notes</h4>
            ...
            <div class="countdown-banner countdown-expired">
                <div class="countdown-icon">ðŸ”’</div>
                <div class="countdown-text">
...
function renderClient() {
    ...
    allSites.forEach(s => {
        // Search filter: null-safe field access â€” also searches bookings (name, phone)
        if (searchVal) {
        ...
        // When searching, bypass date-limit â€” show ALL matching events
        if (searchVal) {
    ...
    Object.keys(grouped).forEach(cat => {
        ...
        grouped[cat].forEach(site => {
            ...
            let banner = isClosed ? (siteTimerExpired && site.aDate >= todayStr ? '<div class="full-banner" style="background:linear-gradient(135deg,#e17055,#d63031)">â° BOOKING CLOSED</div>' : '<div class="full-banner" style="background:#000">CLOSED</div>') : (isFull ? '<div class="full-banner">FULL</div>' : '');
            ...
            if (site.aTimerEnd && !siteTimerExpired && !isFull && site.aDate >= todayStr) {
                ...
                if (remaining) {
                    countdownBadge = `<div class="card-countdown-badge" data-timer-end="${site.aTimerEnd}"><span class="timer-icon-spin">â³</span> <span class="card-timer-text">${remaining}</span></div>`;
                }
            }
            ...
            card.innerHTML = `
                ...
                <div class="card-body">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        ...
                        ${site.aWage ? `<span class="wage-badge" style="background:var(--primary); color:white; padding:4px 8px; border-radius:6px; font-size:0.8rem;">â‚¹${site.aWage}</span>` : ''}
                    </div>
                    ${teamBadge}
                    <div style="margin-top:10px; font-size:0.85rem; color:#666; display:flex; flex-direction:column; gap:4px;">
                        <div>ðŸ“ <strong>${site.aPlaceName}</strong></div>
                        <div>ðŸ—“ï¸ ${formatDisplayDate(site.aDate)} | â° ${site.aTime}</div>
                    </div>
                </div>`;
        ...
        grouped[cat].forEach(s => {
            ...
            card.innerHTML = `
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px; flex-wrap:wrap;">
                    <div>
                        <h3 style="margin:0;">${s.aSitename} (${bookings.length}/${s.aSlots ? s.aSlots : 'âˆž'})</h3>
                        <small>ðŸ“… ${formatDisplayDate(s.aDate)}</small>
                    </div>
                    <div>
...
window.startAttendanceRowEdit = (id) => {
    ...
    setInterval(() => {
        document.querySelectorAll('.card-countdown-badge[data-timer-end]').forEach(badge => {
            ...
            if (!remaining) {
                // Timer expired â€” re-render to show locked state
                if (document.getElementById('sitesGrid')) renderClient();
                return;
            }
